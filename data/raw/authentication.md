# OAuth2 with Password (and hashing), Bearer with JWT tokens[¶](#oauth2-with-password-and-hashing-bearer-with-jwt-tokens "Permanent link"){.headerlink preview=""}

Now that we have all the security flow, let\'s make the application
actually secure, using [JWT]{.abbr title="JSON Web Tokens"} tokens and
...
## About JWT[¶](#about-jwt "Permanent link"){.headerlink preview=""}
...
[https://jwt.io](https://jwt.io/){.external-link target="_blank"}.

## Install `PyJWT`[¶](#install-pyjwt "Permanent link"){.headerlink preview=""}

We need to install `PyJWT` to generate and verify the JWT tokens in
Python.

Make sure you create a [virtual
environment](../../../virtual-environments/){.internal-link preview=""
target="_blank"}, activate it, and then install `pyjwt`:
...
You can read more about it in the [PyJWT Installation
docs](https://pyjwt.readthedocs.io/en/latest/installation.html){.external-link
target="_blank"}.
:::

## Password hashing[¶](#password-hashing "Permanent link"){.headerlink preview=""}
...
### Why use password hashing[¶](#why-use-password-hashing "Permanent link"){.headerlink preview=""}
...
## Install `pwdlib`[¶](#install-pwdlib "Permanent link"){.headerlink preview=""}
...
Make sure you create a [virtual
environment](../../../virtual-environments/){.internal-link preview=""
target="_blank"}, activate it, and then install pwdlib with Argon2:
...
With `pwdlib`, you could even configure it to be able to read passwords
...
## Hash and verify the passwords[¶](#hash-and-verify-the-passwords "Permanent link"){.headerlink preview=""}

Import the tools we need from `pwdlib`.

Create a PasswordHash instance with recommended settings - it will be
used for hashing and verifying passwords.

::: {.admonition .tip}
Tip

pwdlib also supports the bcrypt hashing algorithm but does not include
legacy algorithms - for working with outdated hashes, it is recommended
to use the passlib library.
...
If you check the new (fake) database `fake_users_db`, you will see how
the hashed password looks like now:
`"$argon2id$v=19$m=65536,t=3,p=4$wagCPXjifgvUFBzq4hqe3w$CYaIb8sB+wtD+Vu/P4uod1+Qof8h+1g7bbDlBID48Rc"`.
:::

## Handle JWT tokens[¶](#handle-jwt-tokens "Permanent link"){.headerlink preview=""}
...
And copy the output to the variable `SECRET_KEY` (don\'t use the one in
the example).
...
## Update the dependencies[¶](#update-the-dependencies "Permanent link"){.headerlink preview=""}
...
## Update the `/token` *path operation*[¶](#update-the-token-path-operation "Permanent link"){.headerlink preview=""}
...
::::::::::::::: {.tabbed-set .tabbed-alternate tabs="8:4"}
::: tabbed-labels
Python 3.9+Python 3.8+Python 3.10+ - non-AnnotatedPython 3.8+ -
non-Annotated
:::
...
### Technical details about the JWT \"subject\" `sub`[¶](#technical-details-about-the-jwt-subject-sub "Permanent link"){.headerlink preview=""}

The JWT specification says that there\'s a key `sub`, with the subject
of the token.
...
Then you could add permissions about that entity, like \"drive\" (for
the car) or \"edit\" (for the blog).
...
you could prefix the value of the `sub` key, e.g. with `username:`. So,
...
## Check it[¶](#check-it "Permanent link"){.headerlink preview=""}
...
Notice the header `Authorization`, with a value that starts with
`Bearer`.
:::

## Advanced usage with `scopes`[¶](#advanced-usage-with-scopes "Permanent link"){.headerlink preview=""}
...
**FastAPI** later in the **Advanced User Guide**.
...
**FastAPI** doesn\'t make any compromise with any database, data model
or tool.
...
like `pwdlib` and `PyJWT`, because **FastAPI** doesn\'t require any
complex mechanisms to integrate external packages.
...
You can learn more in the **Advanced User Guide** about how to use
OAuth2 \"scopes\", for a more fine-grained permission system, following
these same standards. OAuth2 with scopes is the mechanism used by many
big authentication providers, like Facebook, Google, GitHub, Microsoft,
X (Twitter), etc. to authorize third party applications to interact with
