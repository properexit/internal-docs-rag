# Bigger Applications - Multiple Files[¶](#bigger-applications-multiple-files "Permanent link"){.headerlink preview=""} {#bigger-applications-multiple-files}
...
## An example file structure[¶](#an-example-file-structure "Permanent link"){.headerlink preview=""}
...
- The `app` directory contains everything. And it has an empty file
  `app/__init__.py`, so it is a \"Python package\" (a collection of
  \"Python modules\"): `app`.
- It contains an `app/main.py` file. As it is inside a Python package (a
  directory with a file `__init__.py`), it is a \"module\" of that
  package: `app.main`.
- There\'s also an `app/dependencies.py` file, just like `app/main.py`,
  it is a \"module\": `app.dependencies`.
- There\'s a subdirectory `app/routers/` with another file
  `__init__.py`, so it\'s a \"Python subpackage\": `app.routers`.
- The file `app/routers/items.py` is inside a package, `app/routers/`,
...
## `APIRouter`[¶](#apirouter "Permanent link"){.headerlink preview=""}
...
But it\'s still part of the same **FastAPI** application/web API (it\'s
part of the same \"Python Package\").

You can create the *path operations* for that module using `APIRouter`.

### Import `APIRouter`[¶](#import-apirouter "Permanent link"){.headerlink preview=""}
...
### *Path operations* with `APIRouter`[¶](#path-operations-with-apirouter "Permanent link"){.headerlink preview=""}
...
So we put them in their own `dependencies` module
(`app/dependencies.py`).
...
[Security utilities](../security/){.internal-link preview=""
target="_blank"}.
:::

## Another module with `APIRouter`[¶](#another-module-with-apirouter "Permanent link"){.headerlink preview=""}
...
So, instead of adding all that to each *path operation*, we can add it
to the `APIRouter`.
...
We can also add a list of `tags` and extra `responses` that will be
applied to all the *path operations* included in this router.

And we can add a list of `dependencies` that will be added to all the
*path operations* in the router and will be executed/solved for each
request made to them.

::: {.admonition .tip}
Tip

Note that, much like [dependencies in *path operation
decorators*](../dependencies/dependencies-in-path-operation-decorators/){.internal-link
preview="" target="_blank"}, no value will be passed to your *path
operation function*.
:::
...
- All these *path operations* will have the list of `dependencies`
  ...
  - If you also declare dependencies in a specific *path operation*,
    **they will be executed too**.
  - The router dependencies are executed first, then the [`dependencies`
    in the
    decorator](../dependencies/dependencies-in-path-operation-decorators/){.internal-link
    preview="" target="_blank"}, and then the normal parameter
    dependencies.
  - You can also add [`Security` dependencies with
    `scopes`](../../advanced/security/oauth2-scopes/){.internal-link
...
require authentication for a whole group of *path operations*. Even if
the dependencies are not added individually to each one of them.
:::

::: {.admonition .check}
Check

The `prefix`, `tags`, `responses`, and `dependencies` parameters are (as
in many other cases) just a feature from **FastAPI** to help you avoid
code duplication.
:::

### Import the dependencies[¶](#import-the-dependencies "Permanent link"){.headerlink preview=""}
...
#### How relative imports work[¶](#how-relative-imports-work "Permanent link"){.headerlink preview=""}
...
- then go to the parent of that package (there\'s no parent package,
  `app` is the top level 😱)\...
- and in there, find the module `dependencies` (the file at
  `app/dependencies.py`)\...
- and from it, import the function `get_token_header`.

That would refer to some package above `app/`, with its own file
`__init__.py`, etc. But we don\'t have that. So, that would throw an
...
### Add some custom `tags`, `responses`, and `dependencies`[¶](#add-some-custom-tags-responses-and-dependencies "Permanent link"){.headerlink preview=""}

We are not adding the prefix `/items` nor the `tags=["items"]` to each
*path operation* because we added them to the `APIRouter`.

But we can still add *more* `tags` that will be applied to a specific
*path operation*, and also some extra `responses` specific to that *path
operation*:
...
## The main `FastAPI`[¶](#the-main-fastapi "Permanent link"){.headerlink preview=""}
...
### Import `FastAPI`[¶](#import-fastapi "Permanent link"){.headerlink preview=""}

You import and create a `FastAPI` class as normally.

And we can even declare [global
dependencies](../dependencies/global-dependencies/){.internal-link
preview="" target="_blank"} that will be combined with the dependencies
for each `APIRouter`:
...
### Import the `APIRouter`[¶](#import-the-apirouter "Permanent link"){.headerlink preview=""}
...
### How the importing works[¶](#how-the-importing-works "Permanent link"){.headerlink preview=""}
...
- look for the subpackage `routers` (the directory at
  `app/routers/`)\...
- and from it, import the submodule `items` (the file at
  `app/routers/items.py`) and `users` (the file at
  `app/routers/users.py`)\...

The module `items` will have a variable `router` (`items.router`). This
is the same one we created in the file `app/routers/items.py`, it\'s an
`APIRouter` object.
...
Modules](https://docs.python.org/3/tutorial/modules.html){.external-link
target="_blank"}.
:::::

### Avoid name collisions[¶](#avoid-name-collisions "Permanent link"){.headerlink preview=""}

We are importing the submodule `items` directly, instead of importing
...
the `router` from `users` would overwrite the one from `items` and we
wouldn\'t be able to use them at the same time.
...
### Include the `APIRouter`s for `users` and `items`[¶](#include-the-apirouters-for-users-and-items "Permanent link"){.headerlink preview=""}

Now, let\'s include the `router`s from the submodules `users` and
`items`:
...
With `app.include_router()` we can add each `APIRouter` to the main
`FastAPI` application.
...
### Include an `APIRouter` with a custom `prefix`, `tags`, `responses`, and `dependencies`[¶](#include-an-apirouter-with-a-custom-prefix-tags-responses-and-dependencies "Permanent link"){.headerlink preview=""}

Now, let\'s imagine your organization gave you the
`app/internal/admin.py` file.
...
and add a `prefix`, `dependencies`, `tags`, etc. directly to the
`APIRouter`:
...
:::: tabbed-block
::: highlight
[app/internal/admin.py]{.filename}
...
But we still want to set a custom `prefix` when including the
`APIRouter` so that all its *path operations* start with `/admin`, we
want to secure it with the `dependencies` we already have for this
project, and we want to include `tags` and `responses`.

We can declare all that without having to modify the original
`APIRouter` by passing those parameters to `app.include_router()`:
...
### Include a *path operation*[¶](#include-a-path-operation "Permanent link"){.headerlink preview=""}
...
and it will work correctly, together with all the other *path
operations* added with `app.include_router()`.

::: {.admonition .info}
Very Technical Details

**Note**: this is a very technical detail that you probably can **just
skip**.

------------------------------------------------------------------------

The `APIRouter`s are not \"mounted\", they are not isolated from the
rest of the application.

This is because we want to include their *path operations* in the
OpenAPI schema and the user interfaces.
...
## Check the automatic API docs[¶](#check-the-automatic-api-docs "Permanent link"){.headerlink preview=""}
...
## Include the same router multiple times with different `prefix`[¶](#include-the-same-router-multiple-times-with-different-prefix "Permanent link"){.headerlink preview=""}

You can also use `.include_router()` multiple times with the *same*
...
## Include an `APIRouter` in another[¶](#include-an-apirouter-in-another "Permanent link"){.headerlink preview=""}

The same way you can include an `APIRouter` in a `FastAPI` application,
...
Make sure you do it before including `router` in the `FastAPI` app, so
that the *path operations* from `other_router` are also included.
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
